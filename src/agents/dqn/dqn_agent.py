...
class DQNAgent(BaseAgent):
  ...
  def __init__(self,
    ...
    self.replay_buffer = SimpleBuffer(max_size=buffer_size)
  ...
  def plan(self, obs):
    ...
    if np.random.randn() < self.epsilon:
      ...
      state = torch.FloatTensor(obs).float().unsqueeze(0).to(__device__)
      qvals = self.qnet1.forward(x=state)
  ...
  def _compute_loss(self, batch):
    ...
    # Calculate Q_1(s,a), Q_2(s,a)
    curr_q1 = self.qnet1.forward(states).gather(1, actions)
    curr_q2 = self.qnet2.forward(states).gather(1, actions)

    # Calculate target Q'_1(s)
    next_q1 = self.qnet1.forward(next_states)
    next_q2 = self.qnet2.forward(next_states)
